...
# convention for assembly/processed files.
#
#     /data/ibis/data/assembly/417879/V06/mri/processed/ibis_417879_V06_t1w_001.mnc
#
...
#
class LorisAssemblyProcessedSshDataProvider < SshDataProvider
  ...
  def impl_sync_to_provider(userfile) #:nodoc:
    ...
    mkdir_command = "mkdir -p #{level4.to_s.bash_escape} >/dev/null 2>&1"
  ...
  def provider_full_path(userfile) #:nodoc:
    ...
    subid,visid = named_according_to_LORIS_convention!(userfile)
    Pathname.new(remote_dir) + subid + visid + "mri" + "processed" + basename
  end
